// ─────────────────────────────────────────────────────────────────────────────
// ExecutiveDashboard.tsx — Phase 2.10 (rev 1)
// ─────────────────────────────────────────────────────────────────────────────
//
// CAPABILITY AUDIT
// ─────────────────────────────────────────────────────────────────────────────
//
// UNCHANGED
//   KPI strip (4 tiles): Open Pipeline · Pending Approvals · Avg Approval
//   Turnaround · Weighted Forecast — all clickable to filter drill-down.
//   Pipeline funnel: Q / D / P / N / W — count, value, bar, stuck, warn.
//   Funnel footer: open total, weighted, stuck count.
//   Approval queues: Finance / Legal (BOTTLENECK) / Manager — pending,
//   overdue, avgH, sla, per-queue drill-down click.
//   Exception types: Discount / Payment terms / Legal/indemnity — count,
//   value, detail note — in collapsible accordion.
//   Projection health: 6 rows — in collapsible "Data freshness" accordion.
//   Access notice: compact strip above drill-down.
//   Drill-down table: 9 columns; sticky opportunity preview panel.
//   Refresh Projection action; Export Summary (conditional).
//
// DRILL PRESETS — 14 total; corrected in rev 1:
//   all       : () => true
//   pipeline  : () => true  (same data, different label context)
//   approvals : pending + overdue only — "Approval action required" —
//               DISTINCT from turnover; clicking "Pending approvals" KPI.
//   turnover  : pending | overdue | sentback | legal | approved — broader
//               "all deals with approval activity" — DISTINCT from approvals;
//               clicking "Avg turnaround" KPI shows full throughput picture.
//   closingQ2 : close < "2026-07-01"
//   stageQ/D/P/N: filter by stageCode — may produce results
//   stageW    : stageCode === "W" — honest real filter; returns empty when
//               exec drill-down data covers open opportunities only; empty
//               state explains this (Closed Won tracked in separate view).
//   apprFin   : approvalLabel starts with "Finance"
//   apprLeg   : approvalLabel includes "Legal"
//   apprMgr   : approvalLabel includes "Manager" — honest real filter;
//               returns empty when no manager-level requests are active;
//               empty state explains this.
//   risk      : riskSev !== "none"
//
// FALSE AFFORDANCES — fixed in rev 1:
//   ALL TEAMS / Q2 2026: converted from interactive-styled chips to
//     non-interactive .exe-context-label spans. No real backend filter
//     exists; they render as honest read-only context indicators.
//   Approvals ›: renders as <button> only when onOpenApprovals is provided;
//     renders as a plain <span> otherwise — no dead click target.
//   Export Summary: renders only when onExport is provided.
//
// REFRESH SEMANTICS — fixed in rev 1:
//   refreshState: "idle" | "loading" | "success" | "error"
//   success: only on resolved onRefresh promise.
//   error:   catch block sets "error"; button shows "✕ Refresh failed —
//     retry"; auto-clears after 5 s. Never shows success on failure.
//
// BACKEND / API CONSTRAINTS (unchanged from Phase 2.10)
//   // CONSTRAINT: reportingRefresh() endpoint not yet implemented.
...
//   // CONSTRAINT: pending imports/merges create known projection lag.
//
// ─────────────────────────────────────────────────────────────────────────────
